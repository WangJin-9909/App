        ...
        try {
            ...
            while ((length = in.read(bytes)) != -1) {
                ...
                if (mIsCanceled || read >= PreloadManager.PRELOAD_LENGTH) {
                    if (mIsCanceled) {
                        AppLogger.i("预加载取消：" + mPosition + " 读取数据：" + read + " Byte");
                    } else {
                        AppLogger.i("预加载成功：" + mPosition + " 读取数据：" + read + " Byte");
                    }
                    break;
                }
            }
        } catch (Exception e) {
            AppLogger.i("预加载异常：" + mPosition + " 异常信息："+ e.getMessage());
            // 关入小黑屋
